...
class DashSaiConfig(ConfBase):

        def __init__(self, params={}, args=None):
                super().__init__('dashsai-config', params, args)

        def generate(self):
                # Pass top-level params to sub-generators.
                # Future - can pass some overridden values if needed.
                enis = saigen.enis_sai.Enis_Sai(self.params_dict, self.args)
                aclgroups = saigen.aclgroups_sai.AclGroups_Sai(self.params_dict, args)
